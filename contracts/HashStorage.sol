...
contract HashStorage {
    ...
    // Events
    event HashStored(uint256 indexed hashId, bytes32 dataHash, address indexed owner);
    ...
    // Get hash data by ID
    function getHash(uint256 _hashId) public view returns (bytes32 dataHash, uint256 timestamp, address owner) {
    ...
    function deleteHash(uint256 _hashId) public {
        require(hashes[_hashId].exists, "Hash does not exist");
        require(hashes[_hashId].owner == msg.sender, "Only owner can delete hash");
    ...
    function verifyHash(uint256 _hashId, bytes32 _dataHash) public view returns (bool) {
